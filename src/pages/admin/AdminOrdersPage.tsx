...
export default function AdminOrdersPage() {
  ...
  const syncStatusToAirtable = async (orderId: string, status: string) => {
    try {
      ...
      console.error('Airtable status sync failed:', e);
    }
  };
          ...
          {selectedOrder && (
            <div className="space-y-4 text-sm">
              <div className="space-y-1">
                <p><strong>অর্ডার আইডি:</strong> {selectedOrder.order_id}</p>
                <p><strong>তারিখ:</strong> {new Date(selectedOrder.created_at).toLocaleString('bn-BD')}</p>
              </div>
              <div className="space-y-1 border-t pt-3">
                <p className="font-bold">কাস্টমার তথ্য</p>
                <p>নাম: {selectedOrder.customer_name}</p>
                <p>ফোন: {selectedOrder.customer_phone}</p>
                <p>ইমেইল: {selectedOrder.customer_email}</p>
                <p>ঠিকানা: {selectedOrder.address}, {selectedOrder.area}, {selectedOrder.city}</p>
              </div>
              ...
              <div className="border-t pt-3 space-y-1">
                <p>সাবটোটাল: {formatBDT(selectedOrder.subtotal)}</p>
                <p>শিপিং: {formatBDT(selectedOrder.shipping_cost)}</p>
                {selectedOrder.discount_amount > 0 && <p>ডিসকাউন্ট: -{formatBDT(selectedOrder.discount_amount)}</p>}
                <p className="font-bold text-lg">মোট: {formatBDT(selectedOrder.total)}</p>
              </div>
              <div className="border-t pt-3 space-y-1">
                <p><strong>পেমেন্ট:</strong> {selectedOrder.payment_method === 'cod' ? 'ক্যাশ অন ডেলিভারি' : selectedOrder.payment_method === 'bkash' ? 'বিকাশ' : 'নগদ'}</p>
                {selectedOrder.transaction_id && <p><strong>TxnID:</strong> {selectedOrder.transaction_id}</p>}
                {selectedOrder.notes && <p><strong>নোট:</strong> {selectedOrder.notes}</p>}
              </div>
            </div>
          )}
        </DialogContent>
